...
const techStack = [
  {
    ...
    skills: [
      ...
      { name: "Shadcn UI", icon: <SiShadcnui className="text-zinc-900 dark:text-white" /> },
    ],
  },
  {
    category: "BACKEND",
    skills: [
      ...
      { name: "Express.js", icon: <SiExpress className="text-zinc-900 dark:text-white" /> },
        ...
        {techStack.map((group) => (
          <div key={group.category} className="tech-row flex flex-col lg:flex-row gap-8 lg:gap-24 items-start">
            ...
            <div className="lg:w-1/4">
              <h3 className="tech-category-label text-5xl md:text-6xl font-black text-zinc-900 dark:text-black/10 dark:text-white/10 tracking-tighter hover:text-zinc-900 dark:text-black/20 dark:text-white/20 transition-colors cursor-default">
                {group.category}
              </h3>
            </div>
            ...
            <div className="flex-1 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6">
              {group.skills.map((skill) => (
                <div 
                  key={skill.name}
                  className="tech-icon-box group flex items-center gap-3 p-3 rounded-2xl bg-black/5 dark:bg-white/5 border border-black/5 dark:border-white/5 hover:border-primary/30 transition-all hover:-translate-y-1 hover:bg-white/[0.08]"
                >
